...
const publicChatRoomModel = require("../models/Message/PublicChatRoomModel");
...
// Chat Room Create Router - Private
...
// Chat Room Create Router - Public
router.post("/public/chatroom/create", checkTokenVerify, async(req,res) => {
    try{
        ...
        const chatRoom_exist = await publicChatRoomModel.findOne({groupName});
        if(chatRoom_exist){
            return res.status(409).send({status: false, message: "Group already exists"});
        }

        await publicChatRoomModel.create({
            groupName,
            groupMessages:[],
            totalMembers: members_list.length,
            members: members_list,
...
// Chat Room Get All Group Router - Public
router.post("/public/chatroom/get-allgroup", checkTokenVerify, async(req,res) => {
    try{
        const chatRooms = await publicChatRoomModel.find().select("-groupMessages");

        // Chat Room Groups Send Successfully
        res.status(200).json({status: true, message: "Chat Room Groups Send Successfully", chatRoomGroups: chatRooms});
...
router.put("/public/chatroom/update", checkTokenVerify, async(req,res) => {
    try{
        ...
        const chatRoom = await publicChatRoomModel.findOne({groupName});
...
// Chat Room Get Message Router - Public
router.post("/public/chatroom/get-message", checkTokenVerify, async(req,res) => {
    try{
...
router.post("/public/usergroup", checkTokenVerify, async(req,res)=>{
  try{
        ...
        const groups = await publicChatRoomModel.find({"members.email": user.email});
...
// Mark As Read - Public 
router.post("/public/chatroom/markasread", checkTokenVerify, async(req,res)=>{
    try{
        // Middleware data
        const user = req.user;
...
// PUSHER -------------> - Public
// Send Message with pusher
router.post("/public/chatroom/sendmessage", async(req,res)=>{
    try{
        let {token, textMessage, groupName} = req.body;
        ...
        pusher.trigger(groupName.replace(/ /g, ""), "public-send-message", {
            message: newMessage,
        });
