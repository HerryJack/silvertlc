...
router.post("/nonprofit-form/submit", async(req,res) => {
    try{
        ...
        if(user.role !== "Corporate User"){
            return res.status(403).json({status: false, message: "Access denied. Insufficient permissions"});
        }

        // File Upload
        const uploadFile_URL = await uploadFileToCloudinary(file.tempFilePath, "Corporate User | Non Profit Form | Files");
        ...
        const noProfit_form = await nonProfile_Form_Model.create({ 
            ...
            uploadFiles : uploadFile_URL.secure_url,
            createdAt: new Date()
        });
...
router.put("/nonprofit-form/update", async(req,res) => {
    try{
        ...
        if (uploadFiles && uploadFiles.length > 0) {
            // If there are files, push them to the uploadFiles array
...
router.post("/postservice-form/submit", async(req,res) => {
    try{
        ...
        // File Upload
        const uploadFile_URL = await uploadFileToCloudinary(file.tempFilePath, "Corporate User | Post Service Form | Files");
        // If File is not Uploaded Successfully
        if(!uploadFile_URL){
            return res.status(400).json({status: false, message: "Error in File Uploading on the Cloudinary"});
        }
...
router.post("/transport-form/submit", async(req,res) => {
    try{
        ...
        // File Upload
        const uploadFile_URL = await uploadFileToCloudinary(file.tempFilePath, "Corporate User | Transport Form | Files");
        // If File is not Uploaded Successfully
        if(!uploadFile_URL){
            return res.status(400).json({status: false, message: "Error in File Uploading on the Cloudinary"});
        }
...
router.post("/service-form/submit", async(req,res) => {
    try{
        ...
        // File Upload
        const uploadFile_URL = await uploadFileToCloudinary(file.tempFilePath, "Corporate User | Service Form | Files");
        // If File is not Uploaded Successfully
        if(!uploadFile_URL){
            return res.status(400).json({status: false, message: "Error in File Uploading on the Cloudinary"});
        }
